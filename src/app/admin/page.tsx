  ...
  return (
    <main className="px-5 py-6 lg:px-8 lg:py-8">
      <section className="rounded-[2rem] border border-[#173b2f]/10 bg-[#fffaf0] p-8 shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-[#31574a]">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_.8fr]">
        <div className="overflow-hidden rounded-[2rem] border border-[#173b2f]/10 bg-[#fffaf0] shadow-sm">
          <div className="flex items-center justify-between border-b border-[#173b2f]/10 px-6 py-5">
            <div>
              <h2 className="text-2xl font-black tracking-[-0.04em]">
                Latest orders
              </h2>
              <p className="mt-1 text-sm text-[#31574a]/70">
          ...
          {latestOrders.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="mt-2 text-sm text-[#31574a]/70">
                Orders will appear here after checkout.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-[#173b2f]/10">
              {latestOrders.map((order) => (
                <Link
                  key={order.id}
                  href={`/admin/orders/${order.id}`}
                  className="grid gap-4 px-6 py-5 transition hover:bg-[#fff7e8] md:grid-cols-[1fr_.7fr_.6fr_auto] md:items-center"
                >
                  <div>
                    <p className="font-black text-[#173b2f]">
                  ...
                  <div>
                    <p className="text-sm text-[#31574a]/70">Items</p>
                  ...
                  <div>
                    <p className="text-sm text-[#31574a]/70">Total</p>
                    <p className="font-black">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-[32px] border border-black/10 bg-[#173b2f] p-6 text-[#f8ead4] shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-[#f8ead4]/60">
                  Orders V2: refunds, cancellations and notes.
                </p>
              </div>
            </div>

            <div className="mt-6 rounded-2xl bg-white/10 p-4 text-sm leading-6 text-[#f8ead4]/75">
...
function StatCard({
  ...
  return (
    <div className="rounded-[28px] border border-[#173b2f]/10 bg-[#fffaf0] p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <p className="text-sm font-bold text-[#31574a]/70">{title}</p>
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#eef7ec] text-[#6f855f]">
          {icon}
        </div>
      </div>

      <p className="mt-5 text-4xl font-black tracking-[-0.05em] text-[#173b2f]">
        {value}
      </p>

      <p className="mt-2 text-sm text-[#31574a]/70">{note}</p>
    </div>
  );
}
...
function MiniMetric({
  ...
  return (
    <div className="flex items-center justify-between rounded-2xl bg-[#f4f6f3] px-4 py-3">
      <span className="text-sm font-bold text-[#31574a]/70">{label}</span>
      <span className="font-black text-[#173b2f]">{value}</span>
    </div>
  );
}
